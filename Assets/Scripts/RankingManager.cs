...
public class RankingManager : MonoBehaviour
{
    ...
    void ObtenerRanking()
    {
        ...
        string sqlQuery = "SELECT * FROM RankingPuntos";
        ...
        while (reader.Read())
        {
            rankings.Add(new Ranking(reader.GetInt32(0), reader.GetInt32(1), reader.GetDateTime(2), reader.GetString(3)));
        }
        reader.Close();
        reader = null;
        //Cerramos la DB
    ...
    public void InsertarPuntos(string n, int s)
    {
        ...
        // Crear la consulta
        dbCommand = dbConnection.CreateCommand();
        string sqlQuery = String.Format("INSERT INTO RankingPuntos(Nombre, Puntos) values(\"{0}\",\"{1}\")", n, s);
        dbCommand.CommandText = sqlQuery;
    ...
    void BorrarPuntos(int id)
    {
        ...
        // Crear la consulta
        dbCommand = dbConnection.CreateCommand();
        string sqlQuery = "DELETE FROM Ranking WHERE ID = \"" + id + "\"";
        dbCommand.CommandText = sqlQuery;
        dbCommand.ExecuteScalar();
        //Cerramos la DB
        CerrarDB();
    ...
    void MostrarRanking()
    {
        ...
        for (int i = 0; i < topRank; i++)
        {
            ...
            if (i < rankings.Count)
            {
                ...
                rankTemp.Nombre, rankTemp.Puntos.ToString());
            }
        }
    }
    ...
    void BorrarPuntosExtra()
    {
        ...
        if (limiteRanking <= rankings.Count)
        {
            ...
            for (int i = 0; i < diferencia; i++)
            {
                ...
                string sqlQuery = "DELETE FROM Ranking WHERE ID = \"" + rankings[i].ID + "\"";
                dbCommand.CommandText = sqlQuery;
                dbCommand.ExecuteScalar();
            }
            //Cerrar DB
